...
date: 2024/11/28
contact: 【公众号】思维兵工厂
description: 部署腾讯云函数，配合笔记卡片微信，将微信推文转为markdown笔记，存储到对象存储中

部署时需要为该函数设置以下环境变量，同时将部署之后得到的url写入主程序配置文件中：
    - storage_type[可选]：值为“qiniu”或“s3”或“webdav”，默认qiniu
    - token：鉴权token[可选]，如果传入，则只有token输入正确的请求才会处理；部署【笔记卡片】微信号功能时，不要传入；
    ...
    - save_note_path：笔记保存路径，必须传入；

该接口只接收POST请求，需要在请求体中传入如下参数：

    - token[可选]：用于鉴权；
    - note_title[可选]：字符串类型；默认为随机字符串；
    - note_content[可选]：字符串类型；笔记内容；

    - note_url[可选]：网址链接，若传入该值，则会忽略 note_title和note_content，改为获取网页内容；
    - url[可选]：与上相同
    - content：文本内容，用于提取URL

    - save_note_path[可选]：笔记保存路径；默认在根目录下创建【000_wechat_article】文件夹
    - note_source[可选]：笔记来源，用于标记笔记属性；默认为：微信
...
class Handler(object):
    ...
    def __init__(self, data: Dict, note_url: str):
        ...
        # 笔记保存路径；默认在根目录下创建
        self.save_note_path: str = os.getenv('save_note_path') or '000_wechat_article'
...
def is_include_url(text: str) -> bool:
    ...
    # 使用正则表达式进行匹配
    match = url_pattern.match(text.replace('收到了聊天记录:', ''))
...
def run():
    # 返回结果
    response = {"continue": False, "message": ''}

    if request.method.lower() != 'post':
        response['message'] = 'Not post request'
        return jsonify(response)

    data = request.get_json()

    article_url = data.get('url') or data.get('content')
    if not article_url or not is_include_url(article_url):
        response['continue'] = True
    else:
        article_url = article_url.replace('收到了聊天记录:', '')
        handler = Handler(data=data, note_url=article_url)
        response['message'] = handler.run()
