...
from module import aligo
from module.aligo.core.Config import *
from module.aligo.error import AligoStatus500, AligoRefreshFailed, AligoFatalError, AligoShareLinkCreateExceedDailyLimit
from module.aligo.types import *
from module.aligo.types.Enum import *
from module.aligo.utils.LoginTimout import LoginTimeout
...
class Auth:
    ...
    def __init__(
        ...
        if logger:
            # 用户传入logger时，使用用户传入的logger
            self.log = logger
        else:
            # 用户没有传入logger，自己创建，并美化样式
        ...
        if self._name.exists():
            # self.log.info(f'加载配置文件 {self._name}')
            self.token = Token(**json.load(self._name.open(encoding='utf8')).get('aligo'))
    ...
    def _save(self):
        ...
        config = json.load(self._name.open(encoding='utf8'))
        config['aligo'] = self.token.to_dict()

        json.dump(config, self._name.open('w', encoding='utf8'), ensure_ascii=False, indent=4)
