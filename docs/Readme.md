...
![image-20231124152546799](assets/Readme/image-20231124152546799.png)

> 注意：
>
> 微信官方限制，当`微信服务器` 先 `自己服务器` 发送消息时，微信服务器在五秒内收不到响应会断掉连接，并且重新发起请求，总共重试三次。
>
> 有些消息的业务处理逻辑会超过5秒钟，从而导致微信服务器接收不到响应。
>
> 本项目使用存储消息处理结果的办法：
>
> - 每次请求到来时，先下载阿里云盘中的用户数据文件，由消息id判断，该消息是否已经处理过了
>   - 如果已经处理过了，直接返回文件中存放的处理结果；
>   - 如果没有处理过，按照程序正常处理；
> - 每次请求处理完成，将处理结果覆盖存储（仅保存最新一次的处理数据）在用户数据文件，上传于阿里云盘，
> - 从而，将处理时间扩展到10秒。即十秒内能处理完成的请求，微信服务器、用户就仍能正常接收到响应。
