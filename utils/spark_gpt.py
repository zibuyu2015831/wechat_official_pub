...
# 实例化时，需要传入一个logger对象，其他配置信息用config.json传入
class SparkGPT(object):

    def __init__(self, conf_dict, logger_obj=None, prompt=None, language="general"):
        """
        实例化时，需要传入存储相关配置信息的字典
        :param conf_dict:
        :param logger_obj:
        :param prompt:
        :param language:
    ...
    def set_max_tokens(self, value):
        try:
            self.save_log('info', f"正在重设max_tokens，传入值为：{value}")
            max_tokens = int(value)

            if 1 <= max_tokens <= 4096:
                self.max_tokens = max_tokens
                self.save_log('info', f"max_tokens重设成功，设置后的值为：{max_tokens}")
                return max_tokens
            else:
                self.save_log('error', f"max_tokens重设失败，传入值--【{max_tokens}】--超出范围")
                print("max_tokens重设失败，max_tokens取值区间为：[1,4096]，请检查传入值")

        except Exception as e:
            self.save_log('error', f"max_tokens重设失败，传入值--【{value}】--非整数")
            print("max_tokens重设失败，传入参数必须是整数")

    def set_top_k(self, value):
        try:
            self.save_log('info', f"正在重设top_k，传入值为：{value}")
            top_k = int(value)
            if 1 <= top_k <= 6:
                self.top_k = top_k
                self.save_log('info', f"top_k重设成功，设置后的值为：{top_k}")
                return top_k
            else:
                self.save_log('error', f"top_k重设失败，传入值--【{top_k}】--超出范围")
                print("top_k重设失败，top_k取值区间为：[1,6]，请检查传入值")

        except Exception as e:
            self.save_log('error', f"top_k重设失败，传入值--【{value}】--非整数字")
            print("top_k重设失败，传入参数必须是整数")

    def set_temperature(self, value):
        try:
            self.save_log('info', f"正在重设temperature，传入值为：{value}")
            temperature = float(value)
            if 0 <= temperature <= 1:
                self.temperature = temperature
                self.save_log('info', f"temperature重设成功，设置后的值为：{temperature}")
                return temperature
            else:
                self.save_log('error', f"temperature重设失败，传入值--【{temperature}】--超出范围")
                print("temperature重设失败，temperature取值区间为：[0,1]，请检查传入值")

        except Exception as e:
            self.save_log('error', f"temperature重设失败，传入值--【{value}】--非浮点型数字")
    ...
    # 处理流式打印效果
    @staticmethod
    def handle_flow_print(status, content):
        if status == 0:
            print(content, end="")
        elif status == 1:
            print(content, end="")
    ...
    def on_message(self, ws, message):
        ...
        if code != 0:
            ...
            self.save_log('critical', f'回答获取失败，响应信息反序列化之后为： {data}')
            error_msg = self.handle_error_code(code)
            self.save_log('critical', f'【错误提示】 --- {error_msg}')
            return
        else:
            ...
            # 进行日志记录
            if status == 0:
                self.save_log('info', "收到第1次回应，内容为：%s" % {content.replace('\n', '\\n')})
            elif status == 1:
                self.save_log('info', "收到第%d次回应，内容为：%s" % (seq + 1, content.replace('\n', '\\n')))
            elif status == 2:
                self.save_log('info', "收到最终回应，内容为：%s" % content.replace('\n', '\\n'))

            # 多轮会话中，流式打印接收到的回答
            if self.flow_print:
                self.handle_flow_print(status, content)
            ...
            if status == 0:  # 0代表首次结果
                ...
                # self.save_log('info', f"整合所有回应，内容为：{self.single_answer}")
                self.save_log('info', "整合所有回应，内容为：%s" % self.single_answer.replace('\n', '\\n'))
    ...
    def get_length(self):
        ...
        for content in self.text:
            ...
            length += len(temp)
        self.save_log('info', f"文本长度为：{length}")
        return length
    ...
    def check_len(self):
        ...
        self.save_log('info', f"开始循环检查文本长度，预设语言为{self.language}，文本长度限制为{self.max_length}")
        while self.get_length() > self.max_length:
            self.save_log('info', f"文本长度超出限制，去除文本第一项")
            del self.text[0]
            self.save_log('info', f"去除之后的文本：{json.dumps(self.text)}")
        self.save_log('info', f"文本长度满足要求")
        return self.text
    ...
    def send(self, ws, *args):
        ...
        self.save_log('info', f"开始尝试建立与讯飞AI的通讯...")
        self.save_log('debug', f"此次请求的请求头数据为：{data}")
        ws.send(data)
    ...
    def make_text(self, role, content):
        ...
        # self.text.append({"role": role, "content": content})
        self.save_log('debug', f"成功构建文本：{json.dumps(self.text, ensure_ascii=False)}")
    ...
    def ask(self, question, flow_print=False):
        self.save_log('info', f"收到用户提问：【{question}】。开始构建与星火大模型通讯的文本...")  # 日志记录
        # self.reset_text()  # 清除历史会话数据
        self.single_answer_data = OrderedDict()  # 创建有序字典，用于存放用户的问题与对应答案的流式信息
